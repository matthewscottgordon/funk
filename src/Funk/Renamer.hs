...
import Funk.Names (UnresolvedName(..),
                   ResolvedName(..),
                   Location(..))
import Funk.Scope (Scope, addNameToScope, createGlobalScope, createScope)
...
rename :: MonadError String m => Module UnresolvedName -> m (Module ResolvedName)
...
addDefToScope :: Scope -> Def UnresolvedName -> Scope
...
renameDef :: MonadError String m => Scope -> Def UnresolvedName ->
             m (Def ResolvedName)
renameDef scope (Def (UnresolvedName fName) ps fBody) = do
...
renameExpr :: MonadError a m => Scope -> Expr UnresolvedName ->
              m (Expr ResolvedName)
renameExpr _ (FloatLiteral v) = return (FloatLiteral v)
renameExpr s (Call (UnresolvedName n) exprs) = do
  n' <- findName s n
  exprs' <- mapM (renameExpr s) exprs
  return (Call n' exprs')
renameExpr s (Op (UnresolvedName n) left right) = do
  n' <- findName s n
  left' <- renameExpr s left
  right' <- renameExpr s right
  return (Op n' left' right')
renameExpr s (VarRef (UnresolvedName n)) = do
  n' <- findName s n
  return (VarRef n')
